...
3. إذا لم تجد الإجابة بوضوح في النص، قل: "لا تتوفر لديّ هذه المعلومة، يُرجى التواصل مع الجامعة مباشرةً."
4. أجب بالعربية فقط في جميع الأحوال.
5. كن مختصراً ودقيقاً — اذكر الأرقام (سعر الساعة / المعدل / المفتاح) بشكل صريح ومباشر.
...
تعليمات صارمة يجب الالتزام بها:
1. أجب **فقط** بناءً على المعلومات الواردة في محتوى الملف أعلاه.
2. لا تستخدم أي معلومات من قاعدة بيانات الجامعة أو أي مصدر آخر.
3. لا تخترع أي معلومة غير موجودة في الملف.
4. إذا لم تجد الإجابة في الملف، قل: "لا تتوفر هذه المعلومة في الملف المُرفق."
5. أجب بالعربية فقط في جميع الأحوال.
6. كن مختصراً ودقيقاً.
"""
...
class IUGChatbot:
    ...
    def chat(self, question: str, session_id: str) -> dict:
        ...
        """
        # ── Fast Retrieval ───────────────────────────────────────────────────
        fast_answer = self.fast_retrieval(question)
        if fast_answer:
            self.push_history(session_id, question, fast_answer)
            return {"answer": fast_answer, "top_chunks": []}
