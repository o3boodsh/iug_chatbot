...
from database import COL_RANKINGS, close as db_close, get_collection, ping
from postgres_db import close_postgres, get_postgres_student, ping_postgres
from postgres_routes import router as pg_router
from uploaded_files_db import close_uploaded, ping_uploaded
...
def startup_event():
    ...
    print("⏳ Verifying PostgreSQL (edupredict_db) connection…")
    if not ping_postgres():
        print("⚠️  PostgreSQL not reachable — EduPredict feature disabled.")
    else:
        print("✅ PostgreSQL (edupredict_db) connected.")
...
def health():
    ...
    return {
        ...
        "postgres_connected":   ping_postgres(),
    }
...
def login(req: LoginRequest):
    ...
    student = get_collection(COL_RANKINGS).find_one({"_id": student_id})
    if student:
    ...
    pg_student = get_postgres_student(student_id)
    if not pg_student:
        raise HTTPException(status_code=401, detail="رقم الهوية غير موجود")

    stored_pin = str(pg_student.get("pin_hash") or "1234")
    if stored_pin != pin:
        raise HTTPException(status_code=401, detail="PIN غير صحيح")

    student_name = pg_student.get("student_name") or f"Student {student_id}"
    return LoginResponse(
        token=_make_token(student_id),
        student_name=student_name,
        message=f"أهلاً {student_name} 👋",
